...
export class EditCourseComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.httpService
      ...
      .pipe(take(1), retry(2))
      .subscribe((e) => {
        const course = e.result;
        course.sections?.forEach((section: SectionModel) => {
          const sectionForm = this.addEditSection(section);
          section.videos?.forEach((video) => {
            (sectionForm.get('videos') as FormArray).push(
              this.addEditVideo(video),
            );
          });
        });
        this.courseData.patchValue(course);
      });
  }
  ...
  addEditSection(sectionModel: SectionModel) {
    const sectionForm = this.fb.group({
      title: ['', Validators.required],
      description: ['', Validators.required],
      thumbnailImage: ['', Validators.required],
    ...
    sectionForm.patchValue(sectionModel);
    this.sections.push(sectionForm);
    return sectionForm;
  }
  ...
  addEditVideo(videoModel: VideoModel) {
    const videoForm = this.getNewVideoFormGroup();
    videoForm.patchValue(videoModel);
    return videoForm;
  }
